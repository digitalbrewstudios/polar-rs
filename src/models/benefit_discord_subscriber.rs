/*
 * Polar API
 *
 * Read the docs at https://polar.sh/docs/api-reference
 *
 * The version of the OpenAPI document: 0.1.0
 *
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct BenefitDiscordSubscriber {
    /// The ID of the benefit.
    #[serde(rename = "id")]
    pub id: String,
    /// Creation timestamp of the object.
    #[serde(rename = "created_at")]
    pub created_at: String,
    #[serde(rename = "modified_at", deserialize_with = "Option::deserialize")]
    pub modified_at: Option<String>,
    #[serde(rename = "type")]
    pub r#type: Type,
    /// The description of the benefit.
    #[serde(rename = "description")]
    pub description: String,
    /// Whether the benefit is selectable when creating a product.
    #[serde(rename = "selectable")]
    pub selectable: bool,
    /// Whether the benefit is deletable.
    #[serde(rename = "deletable")]
    pub deletable: bool,
    /// The ID of the organization owning the benefit.
    #[serde(rename = "organization_id")]
    pub organization_id: String,
    #[serde(rename = "metadata")]
    pub metadata: std::collections::HashMap<String, models::MetadataValue>,
    #[serde(rename = "organization")]
    pub organization: Box<models::Organization>,
    #[serde(rename = "properties")]
    pub properties: Box<models::BenefitDiscordSubscriberProperties>,
}

impl BenefitDiscordSubscriber {
    pub fn new(
        id: String,
        created_at: String,
        modified_at: Option<String>,
        r#type: Type,
        description: String,
        selectable: bool,
        deletable: bool,
        organization_id: String,
        metadata: std::collections::HashMap<String, models::MetadataValue>,
        organization: models::Organization,
        properties: models::BenefitDiscordSubscriberProperties,
    ) -> BenefitDiscordSubscriber {
        BenefitDiscordSubscriber {
            id,
            created_at,
            modified_at,
            r#type,
            description,
            selectable,
            deletable,
            organization_id,
            metadata,
            organization: Box::new(organization),
            properties: Box::new(properties),
        }
    }
}

#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum Type {
    #[serde(rename = "discord")]
    Discord,
}

impl Default for Type {
    fn default() -> Type {
        Self::Discord
    }
}
